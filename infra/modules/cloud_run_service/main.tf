locals {
  create_service_account = var.run_service_account_email == null
...
resource "google_cloud_run_v2_service" "app" {
  ...
  template {
    service_account = local.create_service_account ? google_service_account.app[0].email : var.run_service_account_email
    containers {
      name  = var.name
...
resource "google_service_account" "app" {
  ...
  display_name = "Service Account for Cloud Run - ${var.name}"
  project      = var.project_id
}

resource "google_project_iam_member" "app_roles" {
  for_each = local.create_service_account ? toset(var.service_account_roles) : []
